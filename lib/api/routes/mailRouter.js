...
} from '../../services/mail/imapNetworkSecurity.js';
...
function normalizeAccountInput(input) {
  ...
  if (!Number.isInteger(port) || !ALLOWED_IMAP_PORTS.includes(port)) {
    throw new Error(`IMAP port must be one of: ${ALLOWED_IMAP_PORTS.join(', ')}.`);
  }
  assertSafeImapEndpoint(host, port);
  const retentionDays = Number(input?.retentionDays ?? 90);
  if (!MAIL_RETENTION_OPTIONS.includes(retentionDays)) {
    throw new Error(`Mail retention must be one of: ${MAIL_RETENTION_OPTIONS.join(', ')} days.`);
  }
  return {
    host,
    username,
...
export default async function mailPlugin(fastify) {
  ...
  fastify.post('/account/test', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({ error: publicImapErrorMessage(error) });
    }
  });
  ...
  fastify.post('/sync', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(502).send({ error: publicImapErrorMessage(error) });
    }
  });
  ...
  fastify.get('/invitations', async (request) => getInvitationTracking(request.session.currentUser));

  fastify.put('/invitations/:listingId', async (request, reply) => {
    const raw = request.body?.appointmentAt;
    const appointmentAt = raw == null || raw === '' ? null : Number(raw);
    if (appointmentAt != null && (!Number.isFinite(appointmentAt) || appointmentAt <= 0)) {
      return reply.code(400).send({ error: 'A valid appointment date is required.' });
    }
    const updated = setInvitationAppointment(
      request.session.currentUser,
      String(request.params?.listingId ?? ''),
      appointmentAt,
    );
    if (!updated) return reply.code(404).send({ error: 'Invited listing not found.' });
    return { updated: true };
  });

  fastify.delete('/messages', async (request) => ({
    deleted: deleteAllMailMessages(request.session.currentUser),
  }));
