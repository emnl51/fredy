...
import mcpTokenPlugin from './routes/mcpTokenRouter.js';
import automationPlugin from './routes/automationRouter.js';
import legacyMailPlugin from './routes/legacyMailRouter.js';
import aiAutomationPlugin from './routes/aiAutomationRouter.js';
...
fastify.register(async (app) => {
  ...
  app.register(applicationPlugin, { prefix: '/api/applications' });
  app.register(mcpTokenPlugin, { prefix: '/api/mcp-tokens' });
  app.register(automationPlugin, { prefix: '/api/automation' });
  app.register(legacyMailPlugin, { prefix: '/api/legacy-mail' });
...
// External automation HTTP API (Bearer token auth - no session). This is intentionally separate
// from the session-authenticated UI routes so n8n and similar tools do not need MCP tool schemas.
fastify.register(aiAutomationPlugin, { prefix: '/api/ai' });
