...
import { getEnabledMailAccountsForSync, purgeExpiredMailMessages } from '../storage/mailStorage.js';
 ...
 *
 * @returns {Promise<{accounts:number,succeeded:number,failed:number,purged?:number,skipped:boolean}>}
 */
export async function runMailSyncTask() {
  if (running) return { accounts: 0, succeeded: 0, failed: 0, skipped: true };
  running = true;
  try {
    const purged = purgeExpiredMailMessages();
    ...
    return { accounts: accounts.length, succeeded, failed, purged, skipped: false };
  } catch (error) {
    logger.warn('Scheduled mail sync sweep failed.', error);
    return { accounts: 0, succeeded: 0, failed: 1, purged: 0, skipped: false };
  } finally {
    running = false;
  }
}
