...
function computeMachineId() {
  ...
  // Some hardened containers deny the underlying interface syscall. A community-service
  // User-Agent must never make the whole API or scraper fail to import in that environment.
  let nets;
  try {
 ...
 */
export const selfHostedUserAgent = `Fredy-Self-Hosted (${computeMachineId()}; https://github.com/emnl51/fredy)`;
