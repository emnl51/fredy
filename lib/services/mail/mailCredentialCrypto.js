...
const VERSION = 'v1';
const CONTENT_VERSION = 'v1c';
const CONTENT_AAD = Buffer.from('fredy:mail-content:v1');
...
/**
 * Encrypt private message presentation fields as one authenticated JSON value.
 * Keeping these fields together prevents names, addresses, subjects, and mail
 * bodies from appearing in plaintext database copies and backups.
 *
 * @param {Object} content
 * @returns {string}
 */
export function encryptMailContent(content) {
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv(ALGORITHM, getEncryptionKey(), iv);
  cipher.setAAD(CONTENT_AAD);
  const encrypted = Buffer.concat([cipher.update(JSON.stringify(content), 'utf8'), cipher.final()]);
  const tag = cipher.getAuthTag();
  return [CONTENT_VERSION, iv.toString('base64url'), tag.toString('base64url'), encrypted.toString('base64url')].join(
    '.',
  );
}

/**
 * Decrypt private message presentation fields.
 *
 * @param {string} envelope
 * @returns {Object}
 */
export function decryptMailContent(envelope) {
  const [version, ivValue, tagValue, encryptedValue, ...extra] = String(envelope ?? '').split('.');
  if (version !== CONTENT_VERSION || !ivValue || !tagValue || !encryptedValue || extra.length > 0) {
    throw new Error('The stored mail content has an unsupported format.');
  }
  try {
    const decipher = crypto.createDecipheriv(ALGORITHM, getEncryptionKey(), Buffer.from(ivValue, 'base64url'));
    decipher.setAAD(CONTENT_AAD);
    decipher.setAuthTag(Buffer.from(tagValue, 'base64url'));
    return JSON.parse(
      Buffer.concat([decipher.update(Buffer.from(encryptedValue, 'base64url')), decipher.final()]).toString('utf8'),
    );
  } catch (error) {
    throw new Error('The stored mail content could not be decrypted.', { cause: error });
  }
}
