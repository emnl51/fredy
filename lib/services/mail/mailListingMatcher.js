...
export function inferGermanApplicationUpdate(subject, body) {
  ...
  if (/\bterminbestatigung\b/.test(text) || /\bbesichtigungstermin (?:bestatigen|bestatigt)\b/.test(text)) {
    const dateTime = raw.match(/\b(\d{1,2})\.(\d{1,2})\.(\d{4})\s+(?:um\s+)?(\d{1,2}):(\d{2})\s*(?:uhr)?\b/i);
    if (dateTime) {
      ...
      if (Number.isFinite(appointmentAt)) return { status: 'invited', appointmentAt };
    }
    return { status: 'invited' };
  }

  if (
    /\bunterlagen (?:hochladen|einreichen|ubermitteln)\b/.test(text) ||
    /\b(?:einkommensnachweis|schufa|mietschuldenfreiheit)\b/.test(text)
  ) {
