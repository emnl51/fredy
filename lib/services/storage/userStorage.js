...
import { validateScopedMcpToken } from './mcpTokenStorage.js';
 ...
 *
 * `password` (the hash) and the retired `mcp_token` column are left out of the
 ...
 * MCP tokens are hashed, scoped, revocable credentials stored in `mcp_tokens`.
...
export const validateMcpToken = (token) => {
  return validateScopedMcpToken(token);
};
