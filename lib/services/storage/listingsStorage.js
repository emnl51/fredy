...
export const setListingStatus = (id, status) => {
  ...
  const previous = fromJson(
    SqliteConnection.query(`SELECT status FROM listings WHERE id = @id`, { id })[0]?.status,
    null,
  );
  const appointmentAt =
    ['invited', 'visited'].includes(normalized) && Number.isFinite(Number(previous?.appointmentAt))
      ? Number(previous.appointmentAt)
      : undefined;
  const payload =
    normalized == null
      ? null
      : JSON.stringify({ status: normalized, setAt: Date.now(), ...(appointmentAt ? { appointmentAt } : {}) });
