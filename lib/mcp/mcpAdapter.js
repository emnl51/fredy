...
import { findListingCandidates, proposeApplicationUpdate } from '../services/storage/automationStorage.js';

const mcpJson = (tool, data) => ({
  content: [
    {
      type: 'text',
      text: `**Tool:** ${tool} | **Status:** OK\n\n\`\`\`json\n${JSON.stringify(data, null, 2)}\n\`\`\``,
    },
  ],
});
...
export function createMcpServer() {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        'Fredy MCP Server - query real estate jobs and listings and propose application updates. ' +
        ...
        'For external inbox automation, extract only the minimum facts required, find candidates, read application context, and create a human-review suggestion. Never send full messages, credentials, attachments, phone numbers, or unrelated personal data to Fredy. ' +
  ...
  server.tool(
    ...
    'Return Fredy application statuses and appointment/task actions accepted by propose_application_update.',
    {},
    async (_input, extra) => {
      const { error } = authenticateToolCall(extra, 'list_application_statuses', 'applications:read');
      if (error) return normalizeError(error, 'list_application_statuses');
      return mcpJson('list_application_statuses', {
        statuses: ['applied', 'invited', 'visited', 'documents_sent', 'accepted', 'rejected', 'not_invited'],
        appointmentActions: ['create', 'reschedule', 'cancel'],
        taskTypes: ['complete_form', 'upload_documents', 'confirm_interest', 'book_appointment', 'other'],
      });
    },
  );

  server.tool(
    'find_listing_candidates',
    'Find user-owned Fredy listings that may correspond to an external housing message. ' +
      'Pass extracted facts only; never pass a complete email body. Results are candidates, not a confirmed match.',
    {
      objectReference: z.string().optional().describe('Provider object/reference number'),
      address: z.string().optional().describe('Street, house number and postcode extracted from the message'),
      title: z.string().optional().describe('Listing title extracted from the message'),
      provider: z.string().optional().describe('Housing provider name'),
      limit: z.number().optional().describe('Maximum candidates, default 10 and maximum 20'),
    },
    async (input, extra) => {
      const { user, error } = authenticateToolCall(extra, 'find_listing_candidates', 'listings:read');
      if (error) return normalizeError(error, 'find_listing_candidates');
      return mcpJson('find_listing_candidates', findListingCandidates({ userId: user.id, ...input }));
    },
  );

  server.tool(
    'get_application_context',
    'Read the current status, appointments, tasks and audit events for one user-owned listing before proposing a change.',
    { listingId: z.string().describe('Fredy listing ID') },
    async ({ listingId }, extra) => {
      const { user, error } = authenticateToolCall(extra, 'get_application_context', 'applications:read');
      if (error) return normalizeError(error, 'get_application_context');
      const context = getApplicationContext(listingId, user.id, user.isAdmin);
      if (!context) return normalizeError('Listing not found or not owned by this user.', 'get_application_context');
      return mcpJson('get_application_context', context);
    },
  );

  server.tool(
    'propose_application_update',
    'Create an idempotent application update suggestion for human review in Fredy. ' +
      'This tool never changes listing status or appointments directly. Do not include email bodies, credentials, or personal correspondence.',
    {
      listingId: z.string().describe('Exact Fredy listing ID selected from candidates'),
      expectedStatus: z
        .string()
        .nullable()
        .optional()
        .describe('Current status observed before creating the suggestion'),
      status: z
        .enum(['applied', 'invited', 'visited', 'documents_sent', 'accepted', 'rejected', 'not_invited'])
        .nullable()
        .optional(),
      appointment: z
        .object({
          action: z.enum(['create', 'reschedule', 'cancel']),
          startsAt: z.number().optional().describe('Unix milliseconds with the source timezone already resolved'),
      ...
      tasks: z
        .array(
          z.object({
            type: z.enum(['complete_form', 'upload_documents', 'confirm_interest', 'book_appointment', 'other']),
      ...
      reason: z.string().min(1).describe('Short factual evidence such as object reference and address match'),
      externalEventId: z.string().min(1).describe('Stable Gmail/Outlook message id; stored only as a SHA-256 hash'),
    },
    async ({ listingId, confidence, reason, externalEventId, ...payload }, extra) => {
      const { user, error } = authenticateToolCall(extra, 'propose_application_update', 'applications:propose');
      if (error) return normalizeError(error, 'propose_application_update');
      try {
        const result = proposeApplicationUpdate({
          userId: user.id,
          listingId,
          payload,
          confidence,
        ...
        return mcpJson('propose_application_update', result);
      } catch (proposalError) {
        return normalizeError(proposalError.message, 'propose_application_update');
      }
    },
  );
