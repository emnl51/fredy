...
export const getInvitations = () => xhrGet('/api/mail/invitations').then(json);
export const updateInvitationAppointment = (listingId, appointmentAt) =>
  xhrPut(`/api/mail/invitations/${encodeURIComponent(listingId)}`, { appointmentAt }).then(json);
export const deleteAllMailMessages = () => xhrDelete('/api/mail/messages').then(json);
